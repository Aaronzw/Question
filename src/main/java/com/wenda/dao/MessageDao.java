...
public interface MessageDao {
    ...
    //消息列表
    @Select({"select", INSERT_FIELDS,", count(id) ","FROM (select * from ",TABLE_NAME ,"where to_id=#{userId} or from_id=#{userId} ORDER BY created_date desc)tt  GROUP BY conversation_id ORDER BY created_date asc"})
